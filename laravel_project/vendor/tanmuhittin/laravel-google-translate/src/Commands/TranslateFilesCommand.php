...
{
    //public static $lang_folder_extend = 'vendor/translation/';
    public static $lang_folder_extend = '';

    //public static $lang_folder_package = 'translation::';
    public static $lang_folder_package = '';
    ...
    public function handle()
    {
        ...
        foreach ($this->locales as $locale) {
            ...
            if($this->json){
                ...
                if(!is_dir(resource_path('lang/' . self::$lang_folder_extend . $locale))){
                    mkdir(resource_path('lang/' . self::$lang_folder_extend . $locale));
                }
    ...
    {
        $files = preg_grep('/^([^.])/', scandir(resource_path('lang/' . self::$lang_folder_extend . $this->base_locale)));
        ...
        foreach ($files as $file) {
            ...
            $already_translateds = [];
            if (file_exists(resource_path('lang/' . self::$lang_folder_extend . $locale . '/' . $file . '.php'))) {
                if($this->verbose)
                    $this->line('File already exists: lang/' . self::$lang_folder_extend . $locale . '/' . $file . '.php. Checking missing translations');
                $already_translateds = trans(self::$lang_folder_package . $file, [], $locale);
            }
            if (in_array($file, $this->excluded_files)) {
                continue;
            }
            $to_be_translateds = trans(self::$lang_folder_package . $file, [], $this->base_locale);
            ...
            if(!file_exists(resource_path('lang/' . self::$lang_folder_extend . $locale ))){
                mkdir(resource_path('lang/' . self::$lang_folder_extend . $locale ));
            }
            $file = fopen(resource_path('lang/' . self::$lang_folder_extend . $locale . '/' . $file . '.php'), "w+");
    ...
    public function translate_json_array_file($locale,$stringKeys)
    {
        ...
        if(file_exists(resource_path('lang/' . self::$lang_folder_extend . $locale . '.json'))){
            $json_translations_string = file_get_contents(resource_path('lang/' . self::$lang_folder_extend . $locale . '.json'));
        ...
        $file = fopen(resource_path('lang/' . self::$lang_folder_extend . $locale . '.json'), "w+");
